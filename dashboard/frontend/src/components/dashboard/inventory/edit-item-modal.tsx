...
interface EditItemModalProps {
  ...
  item: any; // Tipo del ítem a editar
}
...
export function EditItemModal({
  ...
  // Cargar datos del ítem cuando se abre el modal
  useEffect(() => {
    if (item && open) {
      setId(item.id || "");
      setName(item.name || "");
      ...
      setMaxStock(item.maxStock || 0);
      setUnit(item.unit?.toLowerCase() || "");
      setLocation(item.location || "");
      setSupplier(
        item.supplier
          ? suppliers.find((s) => s.label === item.supplier)?.value || ""
          : ""
      );
      setNotes(item.notes || "");
  ...
  const handleSubmit = (e: React.FormEvent) => {
    e.preventDefault();
    // Aquí iría la lógica para actualizar el artículo
            ...
            <div className="grid grid-cols-3 gap-4">
              ...
              <div className="grid gap-2">
                ...
                <Input
                  ...
                  min="0"
                  value={minStock}
                  onChange={(e) => setMinStock(Number.parseInt(e.target.value))}
                  required
                />
              </div>
              <div className="grid gap-2">
                ...
                <Input
                  ...
                  min="0"
                  value={maxStock}
                  onChange={(e) => setMaxStock(Number.parseInt(e.target.value))}
                  required
                />
              </div>
            </div>
